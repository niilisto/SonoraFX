...
public class MediaSound {
    ...
    public MediaSound(Context context, float[] vol) {
    ...
    public void play(int loops) {
        if(loops > -1)
            this.nLoops = loops;
        if (state == STATE_PAUSED || state == STATE_PLAYING && mp[mediaPlayerIndex].getAudioSessionId() != -1) {
            mp[mediaPlayerIndex].start();
            mp[mediaPlayerIndex].setVolume(volumes[0], volumes[1]);
            Log.d("MediaSound", "playing");
            state = STATE_PLAYING;
        }
    }
    ...
    private MediaPlayer.OnCompletionListener completionListener = new OnCompletionListener() {
        ...
        public void onCompletion(MediaPlayer curmp) {
            ...
            Log.d("MediaSound", "Media Player " + mpEnds);
            try {
                if (mp[mpNext] != null) {
                    mp[mpNext].release();
                }
