...
public class SurfaceView extends android.view.SurfaceView implements SurfaceHolder.Callback
{
	...
	public void destroySurface() {
		if(!preserve || MMFRuntime.inst.askToDie) {
			if(egl != null) {
				if (eglSurface != null && eglSurface != EGL10.EGL_NO_SURFACE) {
					egl.eglMakeCurrent (eglDisplay, EGL10.EGL_NO_SURFACE, EGL10.EGL_NO_SURFACE, EGL10.EGL_NO_CONTEXT);	    		
					egl.eglDestroySurface(eglDisplay, eglSurface);
	...
	public void onWindowFocusChanged(boolean hasWindowFocus)
	{
		...
		if(hasWindowFocus) 
		{
			MMFRuntime.inst.updateViewport();
			if(!MMFRuntime.OUYA)
				MMFRuntime.inst.container.requestFocus();
		}
		Log.Log("Windows focus change ...");
	...
	public class CTEGLConfigChooser extends ComponentSizeChooser {
		...
		public CTEGLConfigChooser(int mode, int version, boolean withDepthBuffer) {
			super(version, (mode==MODE_565 ? 5: (mode==MODE_888 ? 8:4)), (mode==MODE_565 ? 6: (mode==MODE_888 ? 8:4)), (mode==MODE_565 ? 5: (mode==MODE_888 ? 8:4)),  (mode==MODE_565 ? 0: (mode==MODE_888 ? 8:0)), withDepthBuffer ? 16 : 0, 8);
		}
	}
