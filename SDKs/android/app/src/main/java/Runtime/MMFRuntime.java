...
public abstract class MMFRuntime extends Activity
{
	...
	// 
	private BroadcastReceiver ouyareceiver = new BroadcastReceiver()
	{
	...
	public void die ()
	{
		...
		askToDie = true;
		// issued here since this is only done on low memory
		//this.onDestroy();
		finish();
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		try
		{
			String line = android.os.Build.DEVICE;
		...
		if(!OUYA && !FIRETV && !NEXUSTV && deviceApi > 10)
		{
			if(targetApi > 10)
		...
		if(!OUYA && (app.hdr2Options & CRunApp.AH2OPT_STATUSLINE) == 0 && (app.gaFlags  & CRunApp.GA_NOHEADING) != 0)
		{
			//if(deviceApi > 13)
		...
		if((app.gaFlags  & CRunApp.GA_NOHEADING) != 0)
			getWindow().requestFeature(Window.FEATURE_NO_TITLE);
		else if(deviceApi > 10)
			getWindow().requestFeature(Window.FEATURE_ACTION_BAR);
		...
		//if(!OUYA && !FIRETV && !GOOGLETV) {
		//	adMobLib = CServices.checkPlayService(app);
		//}
		...
		if((app.gaFlags  & CRunApp.GA_NOHEADING) != 0 && (app.hdr2Options & CRunApp.AH2OPT_STATUSLINE) == 0)
		{
			...
			if(deviceApi > 13 && deviceApi < 16) 
			{
				nav_flag |= (2 | 1);
			}
			if(deviceApi > 15 && deviceApi < 19)
			{
				if (OUYA || uiModeManager.getCurrentModeType() == Configuration.UI_MODE_TYPE_TELEVISION)
				{
					nav_flag |= (1024 /*| 256*/ | 4 | 2 );
					Log.Log("Running on a TV Device");
				} 
				else 
				{
					nav_flag |= (1024| 512 | 4 | 2 );
					Log.Log("Running on a non-TV Device");
				}
			}
			if(deviceApi > 18)
				nav_flag |= (4096 | 2048 | 1024 | 512/*| 256*/ | 4 | 2);
			
			setVisibilityUI(false, nav_flag);
		}
	...
	public void updateViewport ()
	{
		...
		case CRunApp.VIEWMODE_ADJUSTWINDOW:
			...
			//Log.Log ("ADJWND: Vw:"+viewportWidth+" Vh:"+viewportHeight+" Scx:"+scaleX+" Scy:"+scaleX+" Cw:"+currentWidth+" Ch:"+currentHeight);
		...
		case CRunApp.VIEWMODE_CENTER: 
			...
			//Log.Log ("CENTER: Vw:"+viewportWidth+" Vh:"+viewportHeight+" Scx:"+scaleX+" Scy:"+scaleX+" Cw:"+currentWidth+" Ch:"+currentHeight);
		...
		case CRunApp.VIEWMODE_FITINSIDE_ADJUSTWINDOW:
			...
			app.updateWindowDimensions(app.gaCxWin, app.gaCyWin);
			//Log.Log ("FITINSIDEADJUST: Vw:"+viewportWidth+" Vh:"+viewportHeight+" Scx:"+scaleX+" Scy:"+scaleX+" Cw:"+currentWidth+" Ch:"+currentHeight);
		...
		case CRunApp.VIEWMODE_FITOUTSIDE:
			...
			app.updateWindowDimensions(app.gaCxWin, app.gaCyWin);
			//Log.Log ("FITOUTSIDE: Vw:"+viewportWidth+" Vh:"+viewportHeight+" Scx:"+scaleX+" Scy:"+scaleX+" Cw:"+currentWidth+" Ch:"+currentHeight);
		...
		case CRunApp.VIEWMODE_STRETCH:
			...
			app.updateWindowDimensions(app.gaCxWin, app.gaCyWin);
			//Log.Log ("STRETCH: Vw:"+viewportWidth+" Vh:"+viewportHeight+" Scx:"+scaleX+" Scy:"+scaleX+" Cw:"+currentWidth+" Ch:"+currentHeight);
	...
	protected void onDestroy() 
	{
		...
		if(checkForPermission(android.Manifest.permission.CLEAR_APP_CACHE))
			ClearAppCache(this);
	...
	{		
		...
		if((app.hdr2Options & CRunApp.AH2OPT_STATUSLINE) == 0) {
	    	getWindow().getDecorView().setSystemUiVisibility(nav_flag);
	...
	public void onBackPressed() {
		...
		if (run.rhNObjects != 0)
		{
			...
			for(CObject object : localObjectList) {
				if(object != null && object instanceof CExtension) {
					isBackPressed &= ((CExtension) object).ext.onBackPressed();
				}
					-- cptObject;
					if(cptObject == 0)
						break;
		...
		if((app.hdr2Options & CRunApp.AH2OPT_DISABLEBACKBUTTON) == 0 && isBackPressed) {
			if(run != null)
				run.rhQuit=CRun.LOOPEXIT_ENDGAME;
	...
	private Runnable displayNavigationBar=new Runnable()
	{

		@Override
		public void run()
		{
			final View decorView = MMFRuntime.inst.getWindow().getDecorView();
	...
	public void VisibiltyUiChange()
	{
		...
		if(deviceApi > 10) {
			decorView.setOnSystemUiVisibilityChangeListener (new View.OnSystemUiVisibilityChangeListener() {
				...
				public void onSystemUiVisibilityChange(int visibility) {
					...
					if(((app.gaFlags & CRunApp.AH2OPT_STATUSLINE ) != 0) || !hideBar || visibility == 0) //|| (app.gaFlags  & CRunApp.GA_NOHEADING) == 0
						return;
					...
					if(deviceApi >= 14 && deviceApi < 19) 
					{
						if (uiModeManager.getCurrentModeType() == Configuration.UI_MODE_TYPE_TELEVISION) {
							...
							a |= ( 1024 | 512 | /*256 |*/ 4 | 2 | 1);
							Log.Log("Running on a non-TV Device");
						}
					}
					if(deviceApi >= 19)
						a |= setImmersiveSticky(); //(4096 | 2048 | 1024 | 512 | /*256 |*/  4 | 2);

					nav_flag = a;
					Log.Log("Visibility change: "+visibility+" and a: "+a);
					if((a & decorView.getSystemUiVisibility()) != 0)
					{
						decorView.removeCallbacks(displayNavigationBar);
						decorView.postDelayed(displayNavigationBar, timeBar);
					}
				}
			});
		}
		...
		{
			if(((app.gaFlags & CRunApp.AH2OPT_STATUSLINE ) == 0) || (app.gaFlags  & CRunApp.GA_NOHEADING) != 0 || hideBar)
			{
				decorView.setSystemUiVisibility(2|1);				
			}
			
		}
	}	
	
    private int setImmersiveSticky() {
    ...
    public void onRequestPermissionsResult(int requestCode, String permissions[], int[] grantResults) {
		...
		if (run == null) {
			...
			if(enabled_obb)
			{
				Log.Log("Starting by onrequestpermission ...");
				try {
					Intent intent = new Intent(this, Class.forName(this.getPackageName()+".Main"));
	...
	public boolean verifyOkPermissionsApi23(HashMap<String, String> permsToVerify) {
		...
		for (String permission : permsToVerify.keySet()) {
			if (deviceApi > 22 && permission != null && api23Util.checkIfPermissionInManifest(permission)
	...
	public int pushForPermissions(HashMap<String, String> permissions, int returnCode) {
		if(deviceApi > 22)
			return api23Util.pushPermissionsApi23(permissions, returnCode);
		return 0;
	}
	...
	public boolean expansionFileDelivered(boolean mIsMain, int mFileVersion, long mFileSize) {
		if(Build.VERSION.SDK_INT > 22)
		{
			...
			if(!verifyOkPermissionsApi23(permissionsApi23))
				pushForPermissions(permissionsApi23, PERMISSIONS_OBB_REQUEST);
			else
				enabled_obb = true;
		}
		else
		...
		if(enabled_obb)
		{
			...
			if (!doesFileExist(this, fileName, mFileSize, false))
				return false;
		}
	...
	///////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//				ActionBar StatusBar and Menus control and extras
	//
	///////////////////////////////////////////////////////////////////////////////////////////////////////
	public void toogleActionBar(boolean showbar) {
		if(!showbar) {
			...
			try {
				...
				if(MMFRuntime.deviceApi > 10 && MMFRuntime.targetApi > 10) {
    ...
    public void setStatusBarColor(int color)
    {
    	if (deviceApi >= 21)
    	{
    		try {
    			Window w = getWindow();   
    			Class<? extends Window> c = w.getClass();
    			Method m = c.getDeclaredMethod("setStatusBarColor", int.class);//get the setStatusBarColor method using reflection
    			m.invoke(w, 0xFF << 24 | color); //invoke it
    		}
    		catch(Throwable e)
    		{
    			Log.Log("Error "+e.getMessage());
    ...
    public void setDisplayTitle(int flag)
    {
    	try
    	{
    		...
    		if(bar != null)
    		{
    			bar.setDisplayShowTitleEnabled((flag == 0 ? true : false ));
     		}
    	}
    	catch(Exception e) 
    	{
   ...
   public void setActionBarTitle(String title)
    ...
    private int getSpecialID(String res)
    {
		...
		if(res.contains("android.") && res.contains("drawable."))
		{
			try {
				Drawable d = null;
				String resource = res.substring(res.lastIndexOf(".")+1, res.length());
 ...
 // This snippet hides the system bars.
    ...
    {
    	nav_flag = 0 // View.SYSTEM_UI_FLAG_LAYOUT_STABLE
				| View.SYSTEM_UI_FLAG_LAYOUT_HIDE_NAVIGATION
				| View.SYSTEM_UI_FLAG_LAYOUT_FULLSCREEN
    ...
    public void toogleNavigationBar(final boolean showBar, final int value) 
    {
    	// The action bar above honeycomb is always present, so it does not need to be hidden only disabled
    	// allowing to correctly updateViewport and maintain the focus, if the action bar is hidden, the first
    	// focus will be for it.
    	...
    	{
    		if(setVisibilityUI(showBar, value))
    		{
    			View v = MMFRuntime.inst.getWindow().getDecorView ();
    ...
    private boolean setVisibilityUI(boolean showBar, int a)
    {
    	boolean ret = false;
		try
		{
