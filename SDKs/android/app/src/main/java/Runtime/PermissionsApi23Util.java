...
public class PermissionsApi23Util {
	...
	private boolean addPermission(List<String>permissionsNeeded, String permission) {
	    if (checkSelfPermissionApi23(permission) != PackageManager.PERMISSION_GRANTED) {
	        ...
	        if (!shouldShowRequestPermissionRationaleApi23(permission))
	        	return false;
	    }
	    return true;
	}
	...
	public void askForPermissionsApi23() {
	    ...
	    if (!asked && permissionsRequested.size() > 0) {
	        ...
	        if (permissionsNeed.size() > 0) {
	            // Need Rationale
	            String message = "Remember to grant access to: ";
	            if(permissionsTitle.size() == 1)
	            	message = message + permissionsTitle.get(0);
	            else {
		            for (int i = 0; i < permissionsTitle.size(); i++)
		                message = message + "\n" +"\t" + permissionsTitle.get(i);
	            }
	            //showMessageOKCancel(message,
	            //        new DialogInterface.OnClickListener() {
	            //            @Override
	            //            public void onClick(DialogInterface dialog, int which) {
	            //                requestPermissionsApi23(permissionsRequested.toArray(new String[permissionsRequested.size()]),
	            //                		returnRequest);
	            //            }
	            //        });
                requestPermissionsApi23(permissionsRequested.toArray(new String[permissionsRequested.size()]),
