...
public class ACT_CREATEBYNAME extends CAct
{
    public override function execute(rhPtr:CRun):void {
        var pName:String= rhPtr.get_EventExpressionString(CParamExpression(this.evtParams[0]));
        ...
        if ((PARAM_POSITION(this.evtParams[1])).read_Position(rhPtr, 0x11, pInfo))
        {
            ...
            for (oiPtr=rhPtr.rhApp.OIList.getFirstOI(); oiPtr!=null; oiPtr=rhPtr.rhApp.OIList.getNextOI())
            {
                if ( oiPtr.oiType>=2)
                {
                    if (oiPtr.oiName.toLowerCase() == pName.toLowerCase())
                        break;
                }
            }
            ...
            var number:int= rhPtr.f_CreateObject(-1, oiPtr.oiHandle, pInfo.x, pInfo.y, pInfo.dir, 0, pInfo.layer, -1);
            if (number >= 0)
            {
