...
package Extensions
{
	...
	public class CRunKcBoxB extends CRunExtension
	{
		...
		public var bToDisplay:Boolean;
				
		public function CRunKcBoxB()
		{
		}
	    ...
	    public override function createRunObject(file:CBinaryFile, cob:CCreateObjectInfo, version:int):Boolean
	    {
	        ...
	        // Get FrameData        
	        var pData:CRunKcBoxBFrameData = null;
	        ...
	        {
	            pData = CRunKcBoxBFrameData(pExtData);
	        }
			...
			if (ho.hoAdRunHeader.rhApp.bUnicode == true)
			{
				textSize=textSize/2;
			}
	        if (textSize != 0)
	        ...
	        if ((rData_dwFlags & FLAG_CONTAINED) != 0)
	        {
	            ...
	            if (this.rContNum != -1)
	            {
	                var rdPtrCont:CRunKcBoxB = CRunKcBoxB(pData.pContainers.get(this.rContNum));
	    ...
	    public override function destroyRunObject(bFast:Boolean):void
	    {
	        ...
	        // Get FrameData
	        var pData:CRunKcBoxBFrameData = CRunKcBoxBFrameData(rhPtr.getStorage(ho.hoIdentifier));
	    ...
	    public override function handleRunObject():int
	    {
	        ...
	        // Positionnne le sprite
	        sprite.x=ho.hoX-rhPtr.rhWindowX+pLayer.x;
	        sprite.y=ho.hoY-rhPtr.rhWindowY+pLayer.y;
			...
			{					
		        if (pData != null)
		        {
		            ...
		            {
		                if (rhPtr.rh2MouseX>=ho.hoX && rhPtr.rh2MouseX<ho.hoX+ho.hoImgWidth && rhPtr.rh2MouseY>=ho.hoY && rhPtr.rh2MouseY<ho.hoY+ho.hoImgHeight)
		                {
							if (oldToolTipZone<0)
							{
								oldToolTipZone=0;
	    ...
	    public function DisplayObject(idApp:CRunApp, rc:CRect, pText:String, hFnt:CFontInfo):void
	    {
	        ...
	        if ((pText.length != 0) && (rData_textColor != COLOR_NONE))
	        {
	            ...
	            if ((rData_dwFlags & FLAG_DISABLED) != 0)
	            {
	                ...
	                textLocation.right++;
	                textLocation.bottom++;
	                drawText(pText, textLocation, hFnt, rData_dwFlags, clr);
...
*/
		...
		public function drawText(text:String, textLocation:CRect, font:CFontInfo, flags:int, clr:int):void
		{
