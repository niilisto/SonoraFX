...
package Extensions {
	...
	public class CRunBox2DGround extends CRunBox2DBaseParent
	{
		public override function createRunObject(file:CBinaryFile, cob:CCreateObjectInfo, version:int):Boolean {
			this.gObstacle = Number(file.readShort());
			this.gDirection = Number(file.readShort());
			this.gFriction = Number(file.readInt() / 100.0);
			this.gRestitution = Number(file.readInt() / 100.0);
