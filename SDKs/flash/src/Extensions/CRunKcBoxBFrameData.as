//----------------------------------------------------------------------------------
//
// CRUNBOXBFRAMEDATA : Objet Background System Box
//
//----------------------------------------------------------------------------------
package Extensions
{
	...
	public class CRunKcBoxBFrameData extends CExtStorage
	{
		...
		public function CRunKcBoxBFrameData()
		{
		}
	    ...
	    public function AddObjAddr(t:int, reObject:CRunKcBoxB):int
	    {
	    	var i:int;
	        if (t == TYPE_OBJECT)
	        {
	    ...
	    public function RemoveObjAddr(t:int, reObject:CRunKcBoxB):void
	    {
	    	var i:int;
	        if (t == TYPE_OBJECT)
	        {
	   ...
	   // Add objects
	    public function AddContainer(re:CRunKcBoxB):int
	    {
	        return AddObjAddr(TYPE_CONTAINER, re);
	    }
	    public function AddObject(re:CRunKcBoxB):int
	    {
	        return AddObjAddr(TYPE_OBJECT, re);
	    }
	    public function RemoveContainer(re:CRunKcBoxB):void
	    {
	        RemoveObjAddr(TYPE_CONTAINER, re);
	    }
	    public function RemoveObjectFromList(re:CRunKcBoxB):void
	    {
	        RemoveObjAddr(TYPE_OBJECT, re);
	    }
	    // Get objects
	    public function GetContainer(re:CRunKcBoxB):int
	    {
	        ...
	        if (this.pContainers != null)
	        {
	            for (i=0; i < this.pContainers.size(); i++)
	            {
	                ...
	                {
	                    var reThisOne:CRunKcBoxB = CRunKcBoxB(this.pContainers.get(i));
	    ...
	    public function GetObjectFromList(x:int, y:int):int
	    {
	        ...
	        if (this.pObjects != null)
	        {
	            for (i = this.pObjects.size() - 1; i >= 0; i--)
	            {
	                ...
	                {
	                    var reThisOne:CRunKcBoxB = CRunKcBoxB(this.pObjects.get(i));
	    ...
	    public function UpdateContainedPos():void//CRunKcBoxB re)
	    {
	    	var i:int;
