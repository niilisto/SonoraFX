...
package Movements
{
	...
	public class CMovePlatform extends CMove
	{
	    ...
	    {
	        hoPtr=ho;
	        var mpPtr:CMoveDefPlatform=CMoveDefPlatform(mvPtr);
			...
			var jump:int=mpPtr.mpJumpControl;
			if (jump>3) 
	            jump=MPJC_DIAGO;
	    ...
	    public override function move():void
	    {
			...
			var joyDir:int=hoPtr.hoAdRunHeader.rhPlayer[hoPtr.roc.rcPlayer-1];				//; Lire le joystick
			calcMBFoot();
	
			// Calcul de la vitesse en X
			// -------------------------
			var xSpeed:int=MP_XSpeed;
			var speed8:int, dSpeed:int;
			if (MP_JumpStopped==0)
			{
	            if (xSpeed<=0)
	            {
	                if ((joyDir&4)!=0)								// Gauche
	                {
	                    ...
	                    if ((hoPtr.hoAdRunHeader.rhFrame.leFlags&CRunFrame.LEF_TIMEDMVTS)!=0)
	                        dSpeed=(Number(dSpeed))*hoPtr.hoAdRunHeader.rh4MvtTimerCoef;
	                    xSpeed-=dSpeed;
			...
			if (sY!=0)
			{
	            var flags:int=0;							//; Flags de signe	
	            if (sX<0)								//; DX negatif?
	            {
	                flags|=1;
	                sX=-sX;
			...
			//
			sX=xSpeed;
			var cosinus:int=Cosinus32[dir];
			var sinus:int=Sinus32[dir];
			if (cosinus<0) 
	            cosinus=-cosinus;
			if (sinus<0) 
	            sinus=-sinus;
			...
			switch (MP_Type)
			{
	            ...
	            case 0:         // MPTYPE_WALK:
					...
					if (tst_SpritePosition(hoPtr.hoX, hoPtr.hoY+10, MP_HTFOOT, CRunFrame.CM_TEST_PLATFORM, true)==false) 
					{
	                    ...
	                    var d:int=y+MP_HTFOOT-1;						//; 15
	                    var pt:CPoint=new CPoint();
	                    mpApproachSprite(x, d, x, y, MP_HTFOOT, CRunFrame.CM_TEST_PLATFORM, pt);
	            ...
	            case 1:         // MPTYPE_CLIMB:
					...
					if (check_Ladder(hoPtr.hoLayer, hoPtr.hoX+MP_XMB, hoPtr.hoY+MP_YMB)==CRun.INTBAD)
					{
	                    ...
	                    if (MP_YSpeed<0)
	                    {
	                        for (sY=0; sY<32; sY++)
	                        {
	                            if (check_Ladder(hoPtr.hoLayer, hoPtr.hoX+MP_XMB, hoPtr.hoY+MP_YMB+sY)!=CRun.INTBAD)
	                            {
	                                hoPtr.hoY+=sY;
	                                break;
	                            }
			...
			if (MP_Type==MPTYPE_WALK || MP_Type==MPTYPE_CROUCH || MP_Type==MPTYPE_UNCROUCH)
			{
	            do
	            {
	                ...
	                {
					    var list:CArrayList=hoPtr.hoAdRunHeader.objectAllCol_IXY(hoPtr, hoPtr.roc.rcImage, hoPtr.roc.rcAngle, hoPtr.roc.rcScaleX, hoPtr.roc.rcScaleY, hoPtr.hoX, hoPtr.hoY+1, pOiColList);
					    if (list!=null && list.size()==1) 
					    {
							var pHo2:CObject=CObject(list.get(0));
							...
							var dx:int=pHo2.hoX-MP_XObjectUnder;
							var dy:int=pHo2.hoY-MP_YObjectUnder;
							MP_XObjectUnder=pHo2.hoX;
							MP_YObjectUnder=pHo2.hoY;
	    ...
	    public override function stop():void
	    {
			...
			var scrX:int=hoPtr.hoX-hoPtr.hoAdRunHeader.rhWindowX;
			var scrY:int=hoPtr.hoY-hoPtr.hoAdRunHeader.rhWindowY;
			var x:int,y:int, dir:int;
			...
			switch (hoPtr.hoAdRunHeader.rhEvtProg.rhCurCode&0xFFFF0000)
			{
			    ...
			    case (-14<<16):	    // CNDL_EXTCOLLISION:
					...
					{
					    mpApproachSprite(scrX, scrY, hoPtr.roc.rcOldX-hoPtr.hoAdRunHeader.rhWindowX, hoPtr.roc.rcOldY-hoPtr.hoAdRunHeader.rhWindowY, MP_HTFOOT, CRunFrame.CM_TEST_PLATFORM, pt);			
					...
					if (MP_Type==MPTYPE_WALK)
					{
					    ...
					    if(mpApproachSprite(scrX, scrY, scrX, scrY-MP_HTFOOT, 0, CRunFrame.CM_TEST_PLATFORM, pt)) //roPtr.rom.MP_HTFOOT
					    {
					    ...
					    if(mpApproachSprite(scrX, scrY, hoPtr.roc.rcOldX-hoPtr.hoAdRunHeader.rhWindowX, hoPtr.roc.rcOldY-hoPtr.hoAdRunHeader.rhWindowY, 0, CRunFrame.CM_TEST_PLATFORM, pt))
					    {
							hoPtr.hoX=pt.x+hoPtr.hoAdRunHeader.rhWindowX;
							hoPtr.hoY=pt.y+hoPtr.hoAdRunHeader.rhWindowY;
							hoPtr.roc.rcChanged=true;
	    ...
	    public function calcMBFoot():void
	    {
			...
			{
	            ifo=new CImage();
	            ifo.width=hoPtr.hoImgWidth;
	            ifo.height=hoPtr.hoImgHeight;
	            ifo.xSpot=hoPtr.hoImgXSpot;
	            ifo.ySpot=hoPtr.hoImgYSpot;
			}
	    ...
	    //-----------------------------------------------------//
	    public function check_Ladder(nLayer:int, x:int, y:int):int
	    {
	        var prc:CRect = hoPtr.hoAdRunHeader.y_GetLadderAt(nLayer, x, y);
			if ( prc!=null )
			{
			    return prc.top;
			}
	    ...
	    public function mpHandle_Background():void
	    {
			...
			// ---------------------------------------
			if (hoPtr.hoAdRunHeader.colMask_TestObject_IXY(hoPtr, hoPtr.roc.rcImage, hoPtr.roc.rcAngle, hoPtr.roc.rcScaleX, hoPtr.roc.rcScaleY, hoPtr.hoX, hoPtr.hoY, 0, CRunFrame.CM_TEST_OBSTACLE)==false) // FRAROT
			{
	            ...
	            if (hoPtr.hoAdRunHeader.colMask_TestObject_IXY(hoPtr, hoPtr.roc.rcImage, hoPtr.roc.rcAngle, hoPtr.roc.rcScaleX, hoPtr.roc.rcScaleY, hoPtr.hoX, hoPtr.hoY, MP_HTFOOT, CRunFrame.CM_TEST_PLATFORM)==false) // FRAROT
	                return; 
			}
			hoPtr.hoAdRunHeader.rhEvtProg.handle_Event(hoPtr, (-13<<16)|(hoPtr.hoType&0x0000FFFF) );	    // CNDL_EXTCOLBACK
	    }
	}
}