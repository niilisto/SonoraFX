package Events
{
	...
	public class CEventProgram
	{
	    ...
	    public static var PARAMCLICK_DOUBLE:int=0x100;
	    
	    public var rhPtr:CRun;
	    public var maxObjects:int;
	    public var maxOi:int=0;
	    public var nPlayers:int;
	    public var nConditions:Array;         // NUMBEROF_SYSTEMTYPES+OBJ_LAST
	    ...
	    public var rh4CheckDoneInstart:Boolean;			// Build92 to correct start of frame with fade in
	    public var rhEventGroup:CEventGroup;				// Current group
	    public var rhCurCode:int;                                       // Current event
	    public var rh4PickFlags:Array;				// 00-31
	    public var rh2ActionLoop:Boolean;			// Actions flag
	    public var rh2ActionOn:Boolean;			// Flag are we in actions?
	    public var rh2EnablePick:Boolean;  			// Are we in pick for actions?
	    public var rh2EventCount:int;				// Number of the event 
	    public var rh2ActionCount:int;				// Action counter
	    public var rh2ActionLoopCount:int;	  		// Action loops counter
	    public var rh4EventCountOR:int;                         // Number of the event for OR conditions
	    public var rh4ConditionsFalse:Boolean;
	    public var rh3DoStop:Boolean;			// Force the test of stop actions
	    public var rh2EventQualPos:CQualToOiList;          // ***Position in event objects
	    public var rh2EventQualPosNum:int;			// ***Position in event objects
	    public var rh2EventPos:CObject;				// ***Position in event objects
	    public var rh2EventPosOiList:int; 		// ***Position in oilist for TYPE exploration 
	    public var rh2EventPrev:CObject;				// ***Previous object address
	    public var rh2EventPrevOiList:CObjInfo;				// ***Previous object address
		...
		public var rhCurParamString:String;
	    public var rh3CurrentMenu:int;				// For menu II events
	    public var rh2CurrentClick:int = -1;			// For click events II
	    public var rh4_2ndObject:CObject;	 		// Collision object address
	    public var bReady:Boolean;
	    public var rh2ShuffleBuffer:CArrayList;	
	    public var rhCurObjectNumber:int;	 		// Object number
	    public var rh1stObjectNumber:int;          // Number, for collisions
	  
	    public var rh2PushedEvents:CArrayList;
	    public var rh2ActionEndRoutine:Boolean;		// End of action routine
						
		public var callEndForEach:Boolean= false;
		public var complexOnLoop:Boolean= false;

		public function CEventProgram()
		{
	    	nConditions=new Array(7+COI.OBJ_LAST);         	// NUMBEROF_SYSTEMTYPES+OBJ_LAST
	    	rhEvents=new Array(COI.NUMBEROF_SYSTEMTYPES+1);
	    	rh4PickFlags=new Array(4);						// 00-31
		}

	    public function bts(array:Array, index:int):Boolean
	    {
	        var d:int=index/32;
	        var mask:int=1<<(index&31);
	        var b:Boolean=(array[d]&mask)!=0;
	        array[d]|=mask;
	        return b;
	    }
	    ...
	    // -------------------------------------------------------------
	    public function evt_FirstObjectFromType(nType:int):CObject
	    {
            var pHo:CObject;
            var oil:int;
	        ...
	        if (nType == -1)
	        {
	            var pHoStore:CObject;
	            var poil:CObjInfo;
	            var bStore:Boolean = true;
	    ...
	    public function evt_NextObjectFromType():CObject
	    {
	        ...
	        var oil:int = rh2EventPosOiList;			// Adresse de l'oilist
	        var nType:int = rhPtr.rhOiList[oil].oilType;
	        oil++;
	        while (oil < rhPtr.rhMaxOI)
	        {
	    ...
	    // ----------------------------------------------------------------------
	    public function evt_SelectAllFromType(nType:int, bStore:Boolean):CObject
	    {
	        var first:int = -1;
	        var evtCount:int = rh2EventCount;
	    ...
	    // -------------------------------------------------------------
	    public function evt_FirstObject(sEvtOiList:int):CObject
	    {
	        var pHo:CObject;
	        ...
	        {
	            // Selectionne TOUS les objets à partir d'un qualifier
	            // ---------------------------------------------------
	            if ((sEvtOiList&0x7FFF)==0x7FFF)	// -1: pas d'objet du tout dans le jeu!
	            {
	                return null;
	            }
	        ...
	        var oilPtr:CObjInfo = rhPtr.rhOiList[sEvtOiList];
	    ...
	    public function qualProc(sEvtOiList:int):CObject
	    {
	        var pHo:CObject;
	        var oilPtr:CObjInfo ;
	        var count:int;
	        ...
	        var qoiList:int;
	        var addCount:int;
	        var pQoi:CQualToOiList = qualToOiList[sEvtOiList & 0x7FFF];
	    ...
	    // -------------------------------------------------
	    public function evt_AddCurrentQualifier(qual:int):void
	    {
	        var pQoi:CQualToOiList = qualToOiList[qual & 0x7FFF];
	        var noil:int = 0;
	        var oilPtr:CObjInfo;
	...
	//            rhPtr.rhOiList[rh2EventPosOiList].oilListSelected=rh2EventPos.hoNextSelected;
	    ...
	    // -----------------------------------------------
	    public function evt_AddCurrentObject(pHo:CObject):void
	    {
	        var oilPtr:CObjInfo = pHo.hoOiList;
	        if (oilPtr.oilEventCount != rh2EventCount)
	        {
	            // Aucune selection
	            oilPtr.oilEventCount = rh2EventCount;
	        ...
	        else
	        {
	            ...
	            var oils:int = oilPtr.oilListSelected;
	            if (oils < 0)
	            {
	                oilPtr.oilListSelected = pHo.hoNumber;
	                oilPtr.oilNumOfSelected += 1;
	    ...
	    // -------------------------------------------
	    public function deselectThem(oil:int):void
	    {
	        var poil:CObjInfo = rhPtr.rhOiList[oil];		// Pointe la liste
	        poil.oilEventCount = rh2EventCount;
	        poil.oilListSelected = -1;
	        poil.oilNumOfSelected = 0;
	    }

	    public function evt_ForceOneObject(oil:int, pHo:CObject):void
	    {
			...
			else
			{
		        ...
		        var pqoi:CQualToOiList = qualToOiList[oil & 0x7FFF];
		        var qoi:int;
	    ...
	    // -----------------------------------------------
	    public function evt_DeleteCurrentType(nType:int):void
	    {
	    ...
	    {
	        var num:int;
	        var pHO:CObject, pHOPrev:CObject;
	        var oil:int;
	        var oilPtr:CObjInfo;
	    ...
	    // ---------------------------------------------------
	    public function get_ExpressionObjects(expoi:int):CObject
	    {
	        ...
	        // ---------------------------------------------------
	        var pQoi:CQualToOiList = qualToOiList[expoi & 0x7FFF];
	        var qoi:int = 0;
	    ...
	    // ----------------------------------------------------------------------
	    public function get_ParamActionObjects(qoil:int, pAction:CAct):CObject
	    {
	        rh2EnablePick = true;
	        var pObject:CObject = get_CurrentObjects(qoil);
	        if (pObject != null)
	        {
	            if (repeatFlag == false)
	            {
	    ...
	    // ----------------------------------------------------------------------
	    public function get_ActionObjects(pAction:CAct):CObject
	    {
	        pAction.evtFlags &= ~CEvent.EVFLAGS_NOTDONEINSTART;
	        rh2EnablePick = true;
	        var qoil:int = pAction.evtOiList;				// Pointe l'oiList
	        var pObject:CObject = get_CurrentObjects(qoil);
	        if (pObject != null)
	        {
	            if (repeatFlag == false)
	    ...
	    // --------------------------------------------------------------------------
	    public function get_CurrentObjects(qoil:int):CObject
	    {
	        if ((qoil&0x8000)==0)
	        {
	    ...
	    // -----------------------------------
	    public function get_CurrentObject(qoil:int):CObject
	    {
	        var pHo:CObject;
	        var oilPtr:CObjInfo = rhPtr.rhOiList[qoil];
	    ...
	    // --------------------------------------------------------------------------
	    public function get_CurrentObjectQualifier(qoil:int):CObject
	    {
	        var pHo:CObject;
	        var next:int, num:int;
	
	        var pqoi:CQualToOiList = qualToOiList[qoil & 0x7FFF];
	        if (pqoi.qoiActionCount != rh2ActionCount)			//; Premiere exploration?
	        {
	            ...
	            // Prendre tous?
	            // ~~~~~~~~~~~~~
	            if (rh2EnablePick)						// Pick autorisé?
	            {
	    ...
	    // ------------------------------------------------------
	    public function qoi_GetNextListSelected(pqoi:CQualToOiList):int
	    {
	        var pos:int = pqoi.qoiActionPos;
	        var qoil:int;
	        var oilPtr:CObjInfo;
	    ...
	    public function qoi_GetFirstListSelected(pqoi:CQualToOiList):int
	    {
	        pqoi.qoiActionPos = 0;
	    ...
	    // -----------------------------------------
	    public function qoi_GetNextList(pqoi:CQualToOiList):int
	    {
	        var pos:int = pqoi.qoiActionPos;
	        var qoil:int;
	        var oilPtr:CObjInfo;
	    ...
	    public function qoi_GetFirstList(pqoi:CQualToOiList):int
	    {
	        pqoi.qoiActionPos = 0;
	    ...
	    // ---------------------------------------------------------------------------
	    public function handle_GlobalEvents(code:int):void
	    {
	        var type:int = code & 0xFFFF;
	        if ((type&0x8000)!=0)
	        {
	        	type=65536-type;
	        }
	        var cond:int = -(code >> 16);
	        var num:int = listPointers[rhEvents[type] + cond];
	        if (num != 0)
	        {
	    ...
	    // ---------------------------------------------------------------------------	
	    public function handle_Event(pHo:CObject, code:int):Boolean
	    {
	        rhCurCode = code; 				// Stocke pour access rapide
	
	        // Des evenements definis?
	        // ~~~~~~~~~~~~~~~~~~~~~~~
	        var cond:int = -(code >> 16);				// Vire le type
	        var num:int = listPointers[pHo.hoEvents + cond];
	    ...
	    // ---------------------------------------------------------------------------
		public function handle_TimerEvents():void 
		{
			var bDelete:Boolean= false;
			var pEvent:TimerEvents=this.rhPtr.rh4TimerEvents;
			var num:int= 0;
			while (pEvent!=null)
			{
				if (this.rhPtr.rhTimer>=pEvent.timer)
				{
					if (pEvent.type==TimerEvents.TIMEREVENTTYPE_ONESHOT)
					{
						...
						num = this.listPointers[this.rhEvents[-COI.OBJ_TIMER] + CCnd.NUM_ONEVENT];
						if (num != 0)
						{
		...
		public function compute_TimerEvents():void
	    {
	        ...
	        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        num = listPointers[rhEvents[-COI.OBJ_GAME] + 1];                // -NUM_START
	        var num2:int, count:int;
	        var evgPtr:CEventGroup, evgGroup:CEventGroup;
	        var evtPtr:CEvent;
	        if (num != 0)
	        {
	            if (rh4CheckDoneInstart)
	            {
	                // Marque DONEBEFOREFADEIN les actions déja effectuees : elle ne seront pas réeffectuées...
	                evgGroup = null;
	                num2 = num;
	                do
	                {
	    ...
	    {
	        var time:int = rhPtr.rhTimer;
	
	        var num:int = listPointers[rhEvents[-COI.OBJ_TIMER] + 3];               // -NUM_TIMER
	        var evtPtr:CEvent;
	        var evgPtr:CEventGroup;
	        if (num != 0)
	        {
	            do
	            {
	                ...
	                var p:PARAM_TIME = PARAM_TIME(evtPtr.evtParams[0]);
	    ...
	    // ---------------------------------------------------------------------------
		public function computeEventFastLoopList(pointers:Array):void
		{
			var bTrue:Boolean;
			var evgPtr:CEventGroup, evgPtr2:CEventGroup;
			var num:int= 0;
			var count:int;
		...
		public function computeEventList(num:int, pHo:CObject):void
	    {
	        var bTrue:Boolean;
	        var evgPtr:CEventGroup, evgPtr2:CEventGroup;
	        var count:int;
	    ...
	    // ---------------------------------------------------------------------------
	    public function call_Actions(pEvg:CEventGroup):void
	    {
	        ...
	        if ((pEvg.evgFlags & CEventGroup.EVGFLAGS_LIMITED) != 0)   				// Les actions limitees et validees
	        {
	            ...
	            {
	                var w_cx:int = rhPtr.rhLoopCount;
	                var w_dx:int = pEvg.evgInhibit;
	            ...
	            {
	                var dwt:int = rhPtr.rhTimer / 10;		// Timer courant / 10
	                var dwmax:int;
	        ...
	        var count:int = 0;                    		// Nombre d'actions
	        var actPtr:CAct;
	        do
	        {
	    ...
	    // ---------------------------------------------------------------------------
	    public function call_Stops(pHo:CObject):void
	    {
	        var oi:int;
	        ...
	        do
	        {
	            ...
	            if (num == (4 << 16) || num == (9 << 16))       // ACTL_EXTSTOP - ACTL_EXTBOUNCE
	            {
	                ...
	                {
	                    var oil:int = actPtr.evtOiList;			// Un qualifier?
	                    if ((oil & 0x8000) != 0)
	                    {
	                        var pq:CQualToOiList = qualToOiList[oil & 0x7FFF];
	                        numOi = 0;
	    ...
	    {
			var rh2ShuffleBuffer_size:int = rh2ShuffleBuffer.size();
	        if (rh2ShuffleBuffer_size <= 1)
	        {
	            return;
	        }
	
	        var num1:int = rhPtr.random(rh2ShuffleBuffer_size);
	        var num2:int;
	        do
	        {
	            num2 = rhPtr.random(rh2ShuffleBuffer_size);
	        } while (num1 == num2);
	
	        var pHo1:CObject = CObject(rh2ShuffleBuffer.get(num1));
	        var pHo2:CObject = CObject(rh2ShuffleBuffer.get(num2));
	    ...
	    public function onMouseButton(b:int, nClicks:int):void
	    {
	        var mouse:int;
	        ...
	        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        rhCurParam0 = mouse;
	        rh2CurrentClick = mouse;			// Pour les evenements II
			...
			for (i=0; i<rhPtr.rhNObjects; i++)
			{
				...
				//rhPtr.getMouseCoords();		// read the mouse position when tap
				
				if (rhPtr.rh2MouseX>=x1 && rhPtr.rh2MouseX<x2 && rhPtr.rh2MouseY>=y1 && rhPtr.rh2MouseY<y2)
				{
		            if ((pHox.hoFlags & CObject.HOF_DESTROYED) == 0)
		            {
						if (pHox.hoType==COI.OBJ_SPR)
						{
							if ((pHox.ros.rsFlags&CRSpr.RSFLAG_COLBOX)==0)
							{
								...
								if (mask.testPoint(x1, y1, rhPtr.rh2MouseX, rhPtr.rh2MouseY))
								{
									list.add(pHox);								
								}
							}
		...
		// when going Mobile
		public function wakeup(vk:int):void
		{
			if (rhPtr != null)
			{
				if (bReady == false)
		...
		*/
	    ...
	    public function onKeyDown(vk:int):void
	    {
	        if (rhPtr != null)
	        {
				//wakeup(vk);
	    ...
	    public function ctoCompare(pZone:PARAM_ZONE, pHo:CObject):Boolean
	    {
	    ...
	    public function count_ZoneTypeObjects(pZone:PARAM_ZONE, stop:int, type:int):CObject
	    {
	        stop++;
	        evtNSelectedObjects = 0;
	
	        var oil:int = 0;
	        var poilLoop:CObjInfo = null;
	        var pHo:CObject;
	        var num:int;
	        do
	        {
	    ...
	    // ----------------------------------------
	    public function count_ObjectsFromType(type:int, stop:int):CObject
	    {
	    ...
	    // ~~~~~~~~~~~~~~~~~~~~~~~~~~
	    public function czaCompare(pZone:PARAM_ZONE, pHo:CObject ):Boolean
	    {
	        if (pHo.hoX < pZone.x1 || pHo.hoX >= pZone.x2)
	        {
	            return false;
	    ...
	    public function select_ZoneTypeObjects(p:PARAM_ZONE, type:int):int
	    {
	        var cpt:int = 0;
	
	        var oil:int = 0;
	        var poilLoop:CObjInfo  = null;
	        var pHoLoop:CObject, pHoFound:CObject;
	        var num:int;
	        do
	        {
	            for (; oil < rhPtr.rhOiList.length; oil++)
	    ...
	    // ---------------------------------------------------
	    public function losCompare(x1:Number, y1:Number, x2:Number, y2:Number, pHo:CObject):Boolean
	    {
	        var delta:Number;
	        var x:int, y:int;
	
	        var xLeft:int = pHo.hoX - pHo.hoImgXSpot;
	        var xRight:int = xLeft + pHo.hoImgWidth;
	        var yTop:int = pHo.hoY - pHo.hoImgYSpot;
	        var yBottom:int = yTop + pHo.hoImgHeight;
	
	        if (x2 - x1 > y2 - y1)
	        {
	            delta = (Number(y2 - y1)) / (Number(x2 - x1));
	            if (x2 > x1)
	            {
	        ...
	        {
	            delta = Number(x2 - x1) / Number(y2 - y1);
	            if (y2 > y1)
	            {
	    ...
	    public function select_LineOfSight(x1:int, y1:int, x2:int, y2:int):int
	    {
	        var cpt:int = 0;
	    ...
	    // ----------------------------------------	
	    // Routine de comptage
	    public function czoCountThem(oil:int, pZone:PARAM_ZONE):int
	    {
	        var count:int = 0;
	        var poil:CObjInfo = rhPtr.rhOiList[oil];
	        var pHo:CObject;
	        var num:int;
	    ...
	    public function count_ZoneOneObject(oil:int, pZone:PARAM_ZONE):int
	    {
	        // Un objet normal
	        if ((oil&0x8000)==0)
	        {
	    ...
	    // Routine de comptage
	    // ~~~~~~~~~~~~~~~~~~~
	    public function countThem(oil:int, stop:int):CObject
	    {
	        var poil:CObjInfo = rhPtr.rhOiList[oil];		// Pointe la liste
	        var pHo:CObject;
	        var num:int;
	    ...
	    public function count_ObjectsFromOiList(oil:int, stop:int):CObject
	    {
	        stop++;									// BX a partir de 1!
	        evtNSelectedObjects = 0;
	        if ((oil&0x8000)==0)
	        ...
	        for (qoi = 0; qoi < pqoi.qoiList.length; qoi += 2)
	        {
	            var pHo:CObject = countThem(pqoi.qoiList[qoi + 1], stop);
	            if (pHo != null)
	            {
	                return pHo;
	            }
	    ...
	    // -----------------------------------------
	    public function pickFromId(value:int):Boolean
	    {
	        var number:int = value & 0xFFFF;
	        ...
	        if (poil.oilEventCount == rh2EventCount)
	        {
	            var next:int = poil.oilListSelected;
	            var pHoFound:CObject = null;
	            while (next >= 0)
	            {
	    ...
	    // ---------------------------------------------------------------------------
	    public function push_Event(routine:int, code:int, lParam:int, pHo:CObject, oi:int):void
	    {
	        var p:CPushedEvent = new CPushedEvent(routine, code, lParam, pHo, oi);
	    ...
	    public function handle_PushedEvents():void
	    {
	        ...
	        {
	            var index:int;
				var rh2PushedEvents_size:int = rh2PushedEvents.size();
	            for (index = 0; index < rh2PushedEvents_size; index++)
	            {
	                var pEvt:CPushedEvent = CPushedEvent(rh2PushedEvents.get(index));
	                if (pEvt != null)
	                {
	                    if (pEvt.code != 0)
	                    {
	    ...
	    {
	        var code:Array = new Array(4);
	        var number:int;
	        var n:int;
	        var eventPos:int
	        while (true)
	        {
	            app.file.readBytesAsArray(code);
	            ...
	            if (code[0] == 0x45 && code[1] == 0x52 && code[2] == 0x3E && code[3] == 0x3E)
	            {
	                ...
	                if (nQualifiers > 0)
	                {
	                    ...
	                    for (n = 0; n < nQualifiers; n++)
	                    {
	                        ...
	                        qualifiers[n].qOi = app.file.readShort();
	                        qualifiers[n].qType = app.file.readShort();
	                    }
	                }
	            }
	    ...
	    public function inactiveGroup(evg:int):int
	    {
	        var bQuit:Boolean;
	        var evgPtr:CEventGroup;
	        var evtPtr:CEvent;
	        var grpPtr:PARAM_GROUP ;
	    ...
	    public function prepareProgram():void
	    {
	        ...
	        var grpPtr:PARAM_GROUP;
	        var evpPtr:CParam;
	        var evg:int, evt:int, evp:int;
	        var groups:CArrayList = new CArrayList();
	        var g:CGroupFind;
	                    ...
	                    for (evt = 0; evt < evgPtr.evgNCond + evgPtr.evgNAct; evt++)
	                    {
	                        ...
	                        if (evtPtr.evtNParams != 0)
	                        {
	                            for (evp = 0; evp < evtPtr.evtNParams; evp++)
	                            {
	                                ...
	                                {
	                                    case 13:	    // PARAM_EVERY
	                                        var p:PARAM_EVERY = PARAM_EVERY(evpPtr);
	                                        p.compteur = p.delay;
	                                        break;
	                                    case 39:	    // PARAM_GROUPOINTER
	                                        var gp:PARAM_GROUPOINTER = PARAM_GROUPOINTER(evpPtr);
	                                        var n:int;
	    ...
	    public function assemblePrograms(run:CRun):void
	    {
	        var evgPtr:CEventGroup;
	        var evtPtr:CEvent;
	        var evpPtr:CParam;
	        ...
	        var o:int, oo:int;
	        var oi:int, oi1:int, oi2:int;
	        var type:int;
	        var nOi:int, i:int, n:int, num:int, type1:int, type2:int;
	        var d:int, evgF:int, evgM:int, q:int, d1:int, d2:int;
	        var code:int;
	        var fWrap:int;
	        var evtAlways:int, evtAlwaysPos:int;
	        var aTimers:int, ss:int;
	        var bOrBefore:Boolean;
	        var cndOR:int;
	        var oilPtr:CObjInfo;
	        var hoPtr:CObject;
	        ...
	        if (nQualifiers > 0)
	        {
	            var count:Array=new Array(nQualifiers);
	            for (q = 0; q < nQualifiers; q++)
	            {
	                oi = (qualifiers[q].qOi) & 0x7FFF;
	                count[q] = 0;
	                for (oil = 0; oil < rhPtr.rhMaxOI; oil++)
	                {
	            ...
	            for (q = 0; q < nQualifiers; q++)
	            {
	                ...
	                if (count[q] != 0)
	                {
	                    qualToOiList[q].qoiList = new Array(count[q] * 2);
	                }
	                    ...
	                    if (rhPtr.rhOiList[oil].oilType == qualifiers[q].qType)
	                    {
	                        for (n = 0; n < 8 && rhPtr.rhOiList[oil].oilQualifiers[n] != -1; n++)
	                        {
	                            if (oi == rhPtr.rhOiList[oil].oilQualifiers[n])
	                            {
	                                ...
	                                qualToOiList[q].qoiList[i * 2 + 1] = oil;
	                                i++;
	                            }
	                        }
	                    }
	        ...
	        colBuffer = new Array(oiMax * 100 * 2 + 1);
	        var colList:int = 0;
			var posStartLoop:CArrayList = new CArrayList();
	        // Boucle d'exploration du programme
	        var evg:int, evt:int, evp:int;
			var pExp:CParamExpression = null;
	        ...
	        for (evg = 0; evg < events.length; evg++)
	        {
	            ...
	            for (evt = 0; evt < evgPtr.evgNAct + evgPtr.evgNCond; evt++)
	            {
					...
					{
						pExp = CParamExpression(evtPtr.evtParams[0]);
						pExp.comparaison = 0;
						if ( pExp.tokens[0].code == ((3 << 16) | 0xFFFF) && pExp.tokens[1].code == 0 )
						{
							posStartLoop.add(new CPosStartLoop(pExp.tokens[0].toString(), pExp));
						}
					}
	                    ...
	                    for (evp = 0; evp < evtPtr.evtNParams; evp++)
	                    {
	                        ...
	                        switch (evpPtr.code)
	                        {
	                            ...
	                            case 54:        // PARAM_FLAG_EXP:
	                                var expPtr:CParamExpression = CParamExpression(evpPtr);
	                                for (n = 0; n < expPtr.tokens.length; n++)
	                                {
	                                    // Un objet avec OI?
	                                    if (EVTTYPE(expPtr.tokens[n].code) > 0)
	                                    {
	                                        var expOi:CExpOi = CExpOi(expPtr.tokens[n]);
	                ...
	                if (type >= COI.OBJ_SPR)
	                {
	                    switch (getEventCode(code))
	                    {
	                        ...
	                        case (-14 << 16):     // CNDL_EXTCOLLISION:
								...
								var pEvpObject:PARAM_OBJECT=PARAM_OBJECT(evtPtr.evtParams[0]);
	        ...
	        // ---------------------------------------------------
	        var aListPointers:Array = new Array(COI.NUMBEROF_SYSTEMTYPES + oiMax + 1);
	        ...
	        var sListPointers:int = ss;
	        listPointers = new Array(sListPointers);
	        ...
	        var wBufNear:Array = new Array(rhPtr.rhFrame.maxObjects);
	        var wPtrNear:int;
	        ...
	        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        var sEventPointers:int = evtAlways + 1;
	        var uil:int;
	        ...
	        eventPointersGroup = new Array(sEventPointers);
	        eventPointersCnd = new Array(sEventPointers);
	        ...
	        var lposBuffer:Array = new Array(sListPointers);
	        for (n = 0; n < sListPointers; n++)
	        {
	            ...
	            for (evt = 0; evt < evgPtr.evgNCond; evt++)
	            {
	                ...
	                if (bOrBefore)
	                {
	                    ...
	                    if ((evtPtr.evtFlags & CEvent.EVFLAGS_ALWAYS) != 0)
	                    {
	                        ...
	                        eventPointersCnd[evtAlwaysPos] = evt;
	                        evtAlwaysPos++;
	                    }
	                    ...
	                    if (type < 0)
	                    {
	                        ...
	                        eventPointersCnd[lposBuffer[lposPtr]] = evt;
	                        lposBuffer[lposPtr]++;
							if (evtPtr.evtCode == ((-16<< 16) | 0xFFFF)) // CCnd.CND_ONLOOP
							{
								...
								//var pExp:CParamExpression= CParamExpression(evtPtr.evtParams[0]);
								pExp = CParamExpression(evtPtr.evtParams[0]);
								if ( pExp.tokens[0].code == ((3<< 16) | 0xFFFF) && pExp.tokens[1].code == 0)
								{
									var pName:String= (EXP_STRING(pExp.tokens[0])).string.toLowerCase();
									var posStartLoop_size:int= posStartLoop.size();
									for (n = 0; n < posStartLoop_size; n++)
									{
										var pPos:CPosStartLoop= CPosStartLoop(posStartLoop.get(n));
										if (CServices.compareStringsIgnoreCase(pPos.m_name,pName))
										{
											var rh4PosOnLoop:CArrayList = rhPtr.rh4PosOnLoop;
											if ( rh4PosOnLoop == null )
												rhPtr.rh4PosOnLoop = rh4PosOnLoop = new CArrayList();
											
											var posOnLoop:CPosOnLoop= null;
											var rh4PosOnLoop_size:int= rh4PosOnLoop.size();
											for (n = 0; n < rh4PosOnLoop_size; n++)
											{
												posOnLoop = CPosOnLoop(rh4PosOnLoop.get(n));
												if (CServices.compareStringsIgnoreCase(pName, posOnLoop.m_name))
													break;
											}
											...
											posOnLoop.m_bOR = bOR;
											pPos.m_expPtr.comparaison = ((n + 1)&0xFFFF);
											break;
										}
									}
								}
	        ...
	        // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	        limitBuffer = new Array(oiMax + 1 + colList / 2);
	        var limitListStart:int = 0;
	        var limitPos:int, limitCur:int;
	        for (oil = 0; oil < rhPtr.rhMaxOI; oil++)
	        {
	            oilPtr = rhPtr.rhOiList[oil];
	    ...
	    public function get_OiListOffset(oi:int, type:int):int
	    {	
	        // Un qualifier
	        if ((oi & COI.OIFLAG_QUALIFIER) != 0)
	        {
	    ...
	    public function isTypeRealSprite(type:int):Boolean
	    {
	        var oil:int;
	        for (oil = 0; oil < rhPtr.rhMaxOI; oil++)
	        {
	    ...
	    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~	
	    public function qual_GetFirstOiList(o:int):int
	    {
	        if ((o & 0x8000) == 0)
	        {
	            qualOilPtr = -1;
	    ...
	    public function qual_GetFirstOiList2(o:int):int
	    {
	        if ((o & 0x8000) == 0)
	        {
	            qualOilPtr2 = -1;
		...
		public function addColList(oiList:int, oiNum:int, oiList2:int, oiNum2:int):void
		{
			// First object = qualifier?
			var qoil:CQualToOiList;
			var pOinOil:int;
			if ( oiNum < 0 )
			{
			...
			// Allocate buffer 
			var colList:Array;
			var oilPtr:CObjInfo=rhPtr.rhOiList[oiList];
	    ...
	    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
	    public function make_ColList1(evgPtr:CEventGroup, colList:int, oi1:int):int
	    {
	        var oi2:int;
	        var flag:int;
	        var code:int;
	        ...
	        for (evt = 0; evt < evgPtr.evgNCond; evt++)
	        {
	            ...
	            if (EVTTYPE(evtPtr.evtCode) >= 2)
	            {
	                ...
	                switch (code)
	                {
	                    ...
	                    case (-12 << 16):         // CNDL_EXTOUTPLAYFIELD:
	                        evpPtr = evtPtr.evtParams[0];
	                        flag = (0x8000 + (PARAM_SHORT(evpPtr)).value);
	                    case (-13 << 16):         // CNDL_EXTCOLBACK:
	                        for (o = qual_GetFirstOiList(evtPtr.evtOiList); o != -1; o = qual_GetNextOiList())
	                        {
	                            oi2 = rhPtr.rhOiList[o].oilOi;
	    ...
	    public function make_ColList2(colList:int, oi1:int, ol:int):int
	    {
	        var oi2:int;
	        var o:int;
	            ...
	            for (evt = 0; evt < evgPtr.evgNAct + evgPtr.evgNCond; evt++)
	            {
	                ...
	                if (evtPtr.evtNParams > 0)
	                {
	                    for (evp = 0; evp < evtPtr.evtNParams; evp++)
	                    {
	                        ...
	                        {
	                            var p:PARAM_SHORT = PARAM_SHORT(evpPtr);
	                            switch (p.value)
	                            {
	                                case 1:
	    ...
	    public function enumSounds(sounds:IEnum):void
	    {
	        var evgPtr:CEventGroup;
	        var evtPtr:CEvent;
	        var evg:int, evt:int, p:int;
	        var pSample:PARAM_SAMPLE;
		...
		// Handle key repeat
		public function HandleKeyRepeat():void
		{
			var evgPtr:CEventGroup;
			var evtPtr:CEvent;
			var evg:int, evt:int, p:int;
	    ...
	    public function evg_FindAction(evgPtr:CEventGroup, n:int):int
	    {
	        return evgPtr.evgNCond + n;
	    }
	
	    public function EVTTYPE(code:int):int
	    {
	    	var c:int=code&0xFFFF;
	    	if ((c&0x8000)!=0)
	    	{
	    		return c-65536;
	    	}
	    ...
	    public function EVTNUM(code:int):int
	    {
			return (code >> 16);
	    }
	
	    public function getEventCode(code:int):int
	    {
		...
		public function endForEach():void {
			...
			var saveForEach:CForEach= this.rhPtr.rh4CurrentForEach;
			var saveForEach2:CForEach= this.rhPtr.rh4CurrentForEach2;
			while(true)
			{
				var pForEach:CForEach=this.rhPtr.rh4ForEachs;
				var pForEach2:CForEach= null;
				var pPrevious:CForEach= null;
				while(pForEach!=null)
				{
					if (pForEach.index<0)
					{
		...
		public function addForEach(pName:String, pHo:CObject, oi:Number):void {
			var pForEach:CForEach= this.rhPtr.rh4ForEachs;
			var pForEachPrevious:CForEach= null;
			while(pForEach!=null)
			{
				if (CServices.compareStringsIgnoreCase(pName, pForEach.name) && pForEach.oi==oi)
				{
